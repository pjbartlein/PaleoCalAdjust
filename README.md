...
The Epstein (1991) interpolation approach is intrinsically periodic, meaning that when applied to interpolate pseudo-daily values from monthly input values for a single year, the interpolated daily values at the end of the year will be consistent with those at the beginning, which is a desirable feature.  However, when sequentially applied to multi-annual time series of monthly data, small discontinuities will arise between years.  In the v1.0 implementation, this discontinuity was removed by smoothing the interpolated daily values at the end and beginning of each year.  The Harzallah (1995) approach, which involves iteratively fitting splines to the input data (and to the residuals from the original fit) is intrinsically not periodic, meaning the interpolated daily values at the end of a single year will not be consistent with those at the beginning.  However, because this approach involves local as opposed to global fitting (as in the Epstein approach), the input data can be padded, either cyclically in the case of a single year's data, or with data from adjacent years in the case of time series.  This effectively eliminates the discontinuity between years.  The Epstein (1991) approach is recommended for adjusting "climatology" data sets (e.g. CMIP/PMIP "Aclim" time-frequency data), while the Harzallah (1995) approach is better suited for adjusting time-series data (e.g. "Amon"-type data sets).
...
With the new infofile format, filenames are listed explicitly, instead of being assembled from the components of a CMIP-style filename.  This broadens the scope of appliations of the program.  For example, the program has been successfully applied to Holocene-long transient simulations with "nonstandard" filenames, dimension names, variable names and missing attributes.  Files that use non-standard dimension names (e.g. `time_counter` and opposed to `time` can generally be handled by using the `ncrename` program in `NCO` to rename the dimensions and attributes. 
...
Minor modifications for consistency with the Bartlein and Shafer (2018, *Geoscientific Model Development Discussions*) manuscript submission.
...
>>>>>>> 8f0ee601fd42cf16cd0c1f7cf1fa8a2bbeb68dba

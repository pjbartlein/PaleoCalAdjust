...
angle_1deg <-  seq(0, 360, by=delta_1deg) * (360/360)
angle1 <- radians(angle_1deg)
npts_1deg <- length(angle1)

delta_5deg <- 5 # 5-degree steps
angle_5deg<-  seq(0, 360, by=delta_5deg) * (360/360) 
angle5 <- radians(angle_5deg)
npts_5deg <- length(angle5)

# small steps
delta_sm <- 0.2
angle_smdeg <- seq(0, 360, by=delta_sm) * (360/360) 
...
monlen_file <- "PMIP4_test_cal_360_day_rmonlen.csv"
monlen <- read.csv(paste(path_prefix, monlen_path, monlen_file, sep=""))
names(monlen)

# output .pngs path
png_path <- paste(path_prefix, "Projects/Calendar/data/Ellipse/orb_monangles/", sep="")
...
for (i in (1:nt)) {
  ...
  png_file <- paste(png_path, "orb_monangles_", png_name, "_ka.png", sep="")
  ...
  b <- a * sqrt(1.0 - ecc^2)
  ae <- a * ecc
  perih_angle_deg <- orb_params$Perih_deg[i] + ((80.0/360.0)*360) # ((80.5/365.0)*360) 
  ...
  for (m in (1:12)) {
    ...
    start_angle <- ( monlen[i, (26 + m)] * (360/360) + 180) # (360/365) + 180) 
    ...
    end_angle <- (monlen[i, (38 + m)] * (360/360) + 180) # (360/365) + 180) 
    ...
    mid_angle <- (monlen[i, (14 + m)] * (360/360) + 180) # (360/365) + 180) 
    ...
    segments(0, 0, x_seg_inner, y_seg_inner, lwd=2, col="gray60")
  ...
  # magick -delay 50 @orb_monlen_files.txt -loop 0 -layers Optimize orb_monlen.gif
